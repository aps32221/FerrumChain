//! ?��?測試 / Unit tests for `pallet-tax` against the mock runtime.
#![cfg(test)]
...
fn sample_invoice(hash: [u8; 32]) -> InvoiceAnchor {
    InvoiceAnchor {
        ...
        // (`AccountId32`), independent of the mock runtime's `T::AccountId = u64`.
...
fn anchor_invoice_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Tax::anchor_invoice(RuntimeOrigin::signed(alice()), anchor.clone()));
...
fn anchor_invoice_rejects_duplicate() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Tax::anchor_invoice(RuntimeOrigin::signed(alice()), anchor.clone()));
        assert_noop!(
            Tax::anchor_invoice(RuntimeOrigin::signed(alice()), anchor),
...
fn withhold_emits_event() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Tax::withhold(RuntimeOrigin::signed(alice()), subject.clone(), TaxKind::Wage, amount));
...
fn file_obligation_stores_record() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Tax::file_obligation(RuntimeOrigin::signed(alice()), obligation.clone()));
...
fn settle_pays_and_marks_settled() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Tax::settle(RuntimeOrigin::signed(alice()), subject.clone(), 5));
...
fn settle_fails_if_not_found() {
    new_test_ext().execute_with(|| {
        let subject = sample_did();
        assert_noop!(
            Tax::settle(RuntimeOrigin::signed(alice()), subject, 99),
...
fn settle_fails_if_already_settled() {
    new_test_ext().execute_with(|| {
        ...
        let subject = sample_did();
        let obligation = sample_obligation(subject.clone(), false);
        assert_ok!(Tax::file_obligation(RuntimeOrigin::signed(alice()), obligation));
        assert_ok!(Tax::settle(RuntimeOrigin::signed(alice()), subject.clone(), 7));
        assert_noop!(
            Tax::settle(RuntimeOrigin::signed(alice()), subject, 7),
...
fn authorize_audit_requires_existing_invoice() {
    new_test_ext().execute_with(|| {
        assert_noop!(
            Tax::authorize_audit(RuntimeOrigin::signed(auditor()), [9u8; 32], [8u8; 32]),
...
fn authorize_audit_records_commitment() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Tax::anchor_invoice(RuntimeOrigin::signed(alice()), anchor));
        assert_ok!(Tax::authorize_audit(RuntimeOrigin::signed(auditor()), [3u8; 32], [4u8; 32]));
        assert_eq!(AuditLog::<Test>::get([3u8; 32]), Some([4u8; 32]));
    });
}

#[test]
fn authorize_audit_rejects_unauthorized() {
    new_test_ext().execute_with(|| {
        let anchor = sample_invoice([5u8; 32]);
        assert_ok!(Tax::anchor_invoice(RuntimeOrigin::signed(alice()), anchor));
        assert_noop!(
            Tax::authorize_audit(RuntimeOrigin::signed(alice()), [5u8; 32], [4u8; 32]),
...
fn set_brackets_requires_governance() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Tax::set_brackets(RuntimeOrigin::signed(alice()), brackets.clone()),
            sp_runtime::DispatchError::BadOrigin
        );
