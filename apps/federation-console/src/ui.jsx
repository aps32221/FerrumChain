...
import { CALLS, PALLET_INDEX, MODULE_NAME } from './chain.js'
...
export function CallTag({ name }) {
  ...
  return (
    <Mono title="pallet.callIndex">
      {MODULE_NAME}.{name} <span className="dim">· {PALLET_INDEX}/{meta.index}</span>
    </Mono>
  )
}
...
function originLabel(origin, T) {
  switch (origin) {
    ...
    case 'Sim':
      return T('模擬(on_initialize)', 'Sim (on_initialize)')
    default:
      return origin
  }
}
